      ...
      {article.img && (
        <section style={{ background: '#000' }}>
          <div style={{ maxWidth: 900, margin: '0 auto' }}>
            <img src={article.img} alt={article.title} style={{ width: '100%', maxHeight: 480, objectFit: 'cover', display: 'block', opacity: 0.9 }} loading="lazy" onError={e => { e.target.style.display = 'none' }} />
          </div>
        </section>
      )}
      ...
      <section style={{ background: '#fff', padding: '60px 64px 80px' }}>
        <div style={{ maxWidth: 720, margin: '0 auto' }}>
          <FadeIn>
            ...
            {/* Content */}
            {hasContent ? (
              <div className="article-body"><ArticleContent content={article.content} /></div>
            ) : (
            ...
            <div style={{ marginTop: 48, paddingTop: 32, borderTop: '1px solid #E8E8E8', display: 'flex', flexWrap: 'wrap', gap: 16, alignItems: 'center' }}>
              ...
              <Link to="/news" className="btn btn-dark" style={{ fontSize: 13 }}>← All Articles</Link>
            </div>
          </FadeIn>
        </div>
        <style>{`@media(max-width:768px){section[style*='60px 64px']{padding:40px 24px 60px!important}section[style*='100px 64px']{padding:80px 24px 48px!important}}`}</style>
      </section>
